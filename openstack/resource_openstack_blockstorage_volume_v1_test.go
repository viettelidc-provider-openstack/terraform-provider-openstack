		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBlockStorageV1VolumeExists("viettelidc_blockstorage_volume_v1.volume_1", &volume),
					testAccCheckBlockStorageV1VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"viettelidc_blockstorage_volume_v1.volume_1", "name", "volume_1"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckBlockStorageV1VolumeMetadata(&volume, "foo", "bar"),
					resource.TestCheckResourceAttr(
						"viettelidc_blockstorage_volume_v1.volume_1", "name", "volume_1-updated"),
				),
			},
		},
	})
...
func testAccCheckBlockStorageV1VolumeDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "viettelidc_blockstorage_volume_v1" {
			continue
		}
