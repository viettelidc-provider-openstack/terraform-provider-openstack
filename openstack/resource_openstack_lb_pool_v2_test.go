		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2PoolExists("viettelidc_lb_pool_v2.pool_1", &pool),
				),
			},
			{
				Config: TestAccLbV2PoolConfigUpdate,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("viettelidc_lb_pool_v2.pool_1", "name", "pool_1_updated"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2PoolExists("viettelidc_lb_pool_v2.pool_1", &pool),
					resource.TestCheckResourceAttr("viettelidc_lb_pool_v2.pool_1", "protocol", "UDP"),
				),
			},
		},
	})
...
func testAccCheckLBV2PoolDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "viettelidc_lb_pool_v2" {
			continue
		}
