...
func computeInterfaceAttachV2DetachFunc(
	computeClient *gophercloud.ServiceClient, instanceID, attachmentID string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to detach viettelidc_compute_interface_attach_v2 %s from instance %s",
		...
		log.Printf("[DEBUG] viettelidc_compute_interface_attach_v2 %s is still active.", attachmentID)
		return nil, "", nil
	}
}
...
func computeInterfaceAttachV2ParseID(id string) (string, string, error) {
	...
	if len(idParts) < 2 {
		return "", "", fmt.Errorf("Unable to determine viettelidc_compute_interface_attach_v2 %s ID", id)
	}
