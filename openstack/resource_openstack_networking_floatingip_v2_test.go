		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("viettelidc_networking_floatingip_v2.fip_1", "description", "test floating IP"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("viettelidc_networking_floatingip_v2.fip_1", "description", "test"),
					resource.TestCheckResourceAttr("viettelidc_networking_floatingip_v2.fip_1", "fixed_ip", "192.168.199.20"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("viettelidc_networking_floatingip_v2.fip_1", "description", ""),
					resource.TestCheckResourceAttr("viettelidc_networking_floatingip_v2.fip_1", "fixed_ip", "192.168.199.10"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2FloatingIPDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "viettelidc_networking_floatingip_v2" {
			continue
		}
...
resource "viettelidc_networking_port_v2" "port_1" {
  admin_state_up = "true"
  network_id = "${viettelidc_networking_subnet_v2.subnet_1.network_id}"
...
resource "viettelidc_networking_floatingip_v2" "fip_1" {
  ...
  fixed_ip = "${viettelidc_networking_port_v2.port_1.fixed_ip.1.ip_address}"
}
...
resource "viettelidc_networking_floatingip_v2" "fip_1" {
  pool = "%s"
  port_id = "${viettelidc_networking_port_v2.port_1.id}"
  fixed_ip = "${viettelidc_networking_port_v2.port_1.fixed_ip.0.ip_address}"
}
`, osExtGwID, osPoolName)
}
...
resource "viettelidc_networking_floatingip_v2" "fip_1" {
  ...
  description = "test"
  subnet_ids = flatten([
    data.viettelidc_networking_network_v2.ext_network.id, # wrong UUID
    data.viettelidc_networking_network_v2.ext_network.subnets,
    data.viettelidc_networking_network_v2.ext_network.id, # wrong UUID again
  ])
}
`, osPoolName)
}
