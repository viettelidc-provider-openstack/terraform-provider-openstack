		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2PortForwardingExists("viettelidc_networking_portforwarding_v2.pf_1", "viettelidc_networking_floatingip_v2.fip_1", &pf),
					resource.TestCheckResourceAttr("viettelidc_networking_portforwarding_v2.pf_1", "internal_port", "25"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2PortForwardingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "viettelidc_networking_portforwarding_v2" {
			continue
		}
...
func testAccCheckNetworkingV2PortForwardingExists(n string, fipID string, kp *portforwarding.PortForwarding) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if found.ID != rs.Primary.ID {
			return fmt.Errorf("viettelidc_networking_portforwarding_v2 not found")
		}
...
resource "viettelidc_networking_portforwarding_v2" "pf_1" {
  protocol = "tcp"
  internal_ip_address = "${viettelidc_networking_port_v2.port_1.fixed_ip[0].ip_address}"
  internal_port = 25
  internal_port_id = "${viettelidc_networking_port_v2.port_1.id}"
  external_port = 2230
  floatingip_id = "${viettelidc_networking_floatingip_v2.fip_1.id}"
  depends_on = [viettelidc_networking_port_v2.port_1, viettelidc_networking_floatingip_v2.fip_1]
}
